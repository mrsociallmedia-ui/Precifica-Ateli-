...
import { Camera, Save, Plus, Trash2, Building2, Calendar, DollarSign, Wallet, Star, Info, Zap, Edit3, X } from 'lucide-react';
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-pink-50 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-[9px] text-gray-400 italic">Ex: 160h = 40h por semana</p>
              </div>
            </div>

            <div className="space-y-6">
              <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                <Wallet size={12} className="text-yellow-500" /> Custos Fixos do Ateliê
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Gastos Fixos Mensais (R$)</label>
                  <input 
                    type="number"
                    placeholder="Luz, Net, Aluguel..."
                    className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-400 font-bold"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Taxa Mensal MEI / Impostos</label>
                  <input 
                    type="number"
                    className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-400 font-bold"
                    value={companyData.meiTax}
                    onChange={e => setCompanyData({...companyData, meiTax: parseFloat(e.target.value) || 0})}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="p-6 bg-yellow-50 rounded-3xl border border-yellow-100 flex items-start gap-4">
               <Info size={20} className="text-yellow-600 shrink-0 mt-1" />
               <p className="text-xs text-yellow-800 font-medium leading-relaxed">
                 O valor da hora é calculado somando o seu <strong>Salário</strong> + <strong>Custos Fixos</strong> + <strong>MEI</strong> e dividindo pelo total de <strong>Horas de Trabalho</strong>. Use este valor para garantir que todos os seus custos sejam cobertos em cada peça produzida.
               </p>
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-gray-800 mb-8 flex items-center gap-3">
               <div className="p-3 bg-yellow-100 text-yellow-600 rounded-2xl">
                 {editingPlatform ? <Edit3 size={20} /> : <Plus size={20} />}
               </div>
            ...
            <form onSubmit={handleSavePlatform} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Nome (Ex: Shopee, Elo7, Ton)</label>
                <input 
                  type="text" required
                  ...
                  placeholder="Nome do canal ou maquininha"
                />
              </div>

              <div className="space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Taxa Cobrada (%)</label>
                <input 
                  type="number" step="0.01" required
                  className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-yellow-400 outline-none font-black text-yellow-600"
                  value={platformFee}
