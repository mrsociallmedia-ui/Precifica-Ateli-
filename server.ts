...
async function startServer() {
  try {
    ...
    // API Route: Buscar dados salvos na nuvem para qualquer usuário (Cloud Restore)
    app.get("/api/user-data/:email", async (req: Request, res: Response) => {
      try {
        ...
        // Tentar busca exata e busca case-insensitive (ilike)
        let appState = null;
        let updatedAt = null;

        try {
          const fetchRes = await fetch(`${primarySupabaseUrl}/rest/v1/user_data?user_email=ilike.${encodeURIComponent(rawEmail)}&select=app_state,updated_at&limit=1`, {
            headers: {
          ...
          console.warn("Aviso ao buscar dados do usuário no Supabase via server:", fetchErr);
        }

        if (!appState) {
          return res.json({ 
            found: false, 
            message: "Nenhum dado encontrado na nuvem para este e-mail ainda.",
            app_state: null 
          });
        }
        ...
        console.warn("Erro ao buscar dados do usuário na nuvem:", err);
        return res.status(500).json({ error: err?.message || "Erro interno ao buscar dados na nuvem." });
      }
    });

    // API Route: Salvar/Atualizar dados do usuário na nuvem (Cloud Backup)
    app.post("/api/user-data", async (req: Request, res: Response) => {
      try {
        const { user_email, app_state } = req.body;
        const rawEmail = String(user_email || '').trim().toLowerCase();

        if (!rawEmail || !app_state) {
          return res.status(400).json({ error: "E-mail do usuário e app_state são obrigatórios." });
        }
        ...
        if (!saveRes.ok) {
          ...
          console.warn("Aviso ao salvar user_data via server:", errText);
          return res.status(saveRes.status).json({ error: "Erro ao persistir no banco de dados.", details: errText });
        }

        return res.json({ success: true, user_email: rawEmail, updated_at: new Date().toISOString() });
      } catch (err: any) {
        console.warn("Erro ao salvar dados na nuvem via server:", err);
        return res.status(500).json({ error: err?.message || "Erro interno ao salvar dados na nuvem." });
      }
    });
